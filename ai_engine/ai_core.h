...
public:
    ...
    wstring GenerateEventDescription(PlayerContext& player) {
        ...
        if (player.realm >= 11 &&
            ...
            legacyHint = L"你几乎本能地看出对方肩肘间的旧习，这种熟悉感不像今生才有。";
    ...
    void WritePromptFile(PlayerContext& player) {
        ...
        ss << L"- 必须尊重当前境界名称和境界编号：金丹以后不要再写测灵、入门试炼、外门候选、内门名额、杂役牌、少年道途等低阶入门桥段；大乘、渡劫、仙界、道祖阶段要写对应高度的宗门旧债、天劫、仙门、掌道、权柄与大道压力。\n";
        ss << L"- 可以写长辈认可、同辈嫉妒、被人巴结、遭人欺压、暗中试探、隐藏修为，但不要写成旁白总结。\n";
        ss << L"- 如果上下文出现“本世人脉”，优先复用其中的人名、态度和恩怨，让 NPC 像持续存在的人，不要每次都换成陌生路人。\n";
        ss << L"- 如果上下文出现人物状态或近日风声，要按该 NPC 的立场写即时反应：长辈可护短夸赞，同辈可嫉妒酸言，执事可冷脸卡资源，仇家可嘴硬设局；不要写成客气旁白。\n";
        ss << L"- NPC 可以傲慢、护短、嘴硬、暗讽、试探或押注，但不要变成无意义骂街；情绪必须服务修仙关系和后续取舍。\n";
        ss << L"- 只能写玩家当场能听见、看见或事后听到的风声，不要输出“接下来”“下一步”“多半会”“想要”“忌惮”“台词参考”“人物动机参考”等后台规划词。\n";
        ss << L"- 可以让 NPC 追求目标、回避风险、推进自己的局，但必须通过当场动作、眼神、传闻或台词表现，不要替玩家预告未来安排。\n";
        ...
        ss << L"- 如果上下文出现“本世器物”，可以让这些兵刃或法宝在今生事件中发挥作用；它们有器境，可被温养、重铸、留下器痕或成为本命至宝道胚。\n";
        ss << L"- 普通兵刃、丹药、材料和未达通天层次的当世法宝不能稳定跨世；死亡或转世后多半失散，只留下记忆、因果、器痕或道胚。\n";
        ss << L"- 后天通天灵宝可跨纪元长存，但缺少先天一气，久无人温养会衰退；先天通天灵宝自带先天一气，在多数修士眼中已是器物顶点，不要写成鸿蒙至宝低配。\n";
        ss << L"- 本命至宝是修士后天温养出的道途器物，最契合主人，可一路成长为后天通天灵宝；天道境道祖的本命至宝可融万道，后天抵达鸿蒙至宝水平。\n";
        ss << L"- 主角第一世自带黑白伴生玉佩；它和转世记忆有关，但主角不知道真相。它会在死亡后自发护住部分记忆与未竟因果，入世年月也可能不稳定；事件里可以写玉佩发热、梦中玉意、阴阳玉痕或轮回回响，不要直接揭示它的鸿蒙至宝真身。\n";
        ...
        ss << L"- 九大鸿蒙至宝是创世级恒在之物，知者极少；低境界不能占有、装备或主动调用，道祖以上才可在相性、因果和掌道足够时尝试认主、装备和调用权柄。\n";
        ss << L"- 九大鸿蒙至宝各有固定权柄：如果上下文给出某件至宝及其大道，必须按该权柄写，不要把九件混成同一种万能法宝。\n";
        ss << L"- 如果上下文出现“鸿蒙天象”或“天象影响”，优先按本世对应至宝、所映大道和当世影响写事件，不要泛化成普通法宝。\n";
        ss << L"- AI 可以写鸿蒙至宝相关剧情，但不能随口发放最高至宝；是否认主、装备、执掌、消耗和反噬必须服从上下文里的游戏规则与当前状态。\n";
        ss << L"- 如果上下文显示鸿蒙状态为未见、见影或留印，只能写投影、线索、参悟、拒绝或遥远因果；如果显示已认主、已装备或已执掌，才可写调用权柄的情绪、代价和余波。\n";
        ss << L"- 道祖-天道境的意义是掌尽诸道、映照凌驾万道的力量；可毁至宝只是力量映射，不是道途目标。\n";
    ...
    wstring GenerateOutcome(PlayerContext& player, int choiceIndex, bool success,
        ...
        if (success) {
            ...
            if (touchesJade) {
                ...
                ss << L"你没有把线索当成偶然，而是让它并入本世主线，暗处人和事都被牵出一缕回声。";

...
function Build-PortablePrompt {
    ...
    Add-ClippedPromptLine $out "优先延续本世人脉、家世、最近记忆、当前世界和未收束线头；NPC要像活人一样有情绪、目标和试探。" 220
    Add-ClippedPromptLine $out "只写玩家当场能看见或事后听到的风声；不要输出接下来、下一步、多半会、想要、忌惮、台词参考、人物动机参考。" 220
    ...
    $worldPatterns = @(
        ...
        "^- 本世主线:",
        "^- 本世势力牵连:",
        "^\s*\* (伴生玉佩|玉意梦兆|师尊余波|师尊压目|人情生温|人情生隙|旧玉微温|旧玉受扰|旧因回响|旧债压身|势力牵连|本世线索余波|天机生痕)",
        "^- 天机底线:",
...
function Test-ContextLabelLine {
    ...
    if ($clean -match "^(玩家|境界名称|境界|因果|年龄|此世家世|家世|人情风波|轮回传承|当前世界|最近记忆|本世人脉|本世器物|当前继承的传承|失传古法当世解读|大道特性|寿元压力|隐藏设定|时代纪元|重大事件|事件影响|近年大事|世界时间|伴生玉佩|父亲|母亲|养育者|轮回余烬|本命法宝器纹|器痕归灵|天机底线|近期因果摘要|下一处因果钩子|可延续因果|未收束线头|人情压力|NPC近况|NPC 近况|人物风声|势力压力|人物动机参考|人物表面状态|关系数值|稳定设定|游戏规则)\s*[:：]") { return $true }
    if ($clean -match "(境界名称|因果|本世器物|最近记忆|轮回传承|当前世界|本世人脉|隐藏设定|轮回余烬|本命法宝器纹|器痕归灵|当前继承的传承|天机底线|近期因果摘要|下一处因果钩子|可延续因果|未收束线头|人情压力|NPC近况|NPC 近况|人物风声|势力压力|人物动机参考|人物表面状态|关系数值|稳定设定|游戏规则)\s*[:：]") { return $true }
    if ($clean -match "(台词参考|想要.+忌惮|忌惮.+下一步|接下来|多半会|下一步.+此人|AI 可以|C\+\+|provider|GenericAgent)") { return $true }

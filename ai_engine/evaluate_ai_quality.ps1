...
function Test-ContextLabelLine {
    param([string]$Text)
    if ([string]::IsNullOrWhiteSpace($Text)) { return $false }
    if ($Text -match "^(玩家|境界名称|境界|因果|年龄|此世家世|家世|人情风波|轮回传承|当前世界|最近记忆|本世人脉|本世器物|当前继承的传承|失传古法当世解读|大道特性|寿元压力|隐藏设定|时代纪元|重大事件|事件影响|近年大事|世界时间|伴生玉佩|父亲|母亲|养育者|轮回余烬|本命法宝器纹|器痕归灵|天机底线|近期因果摘要|下一处因果钩子|未收束线头|人情压力|NPC近况|势力压力)\s*[:：]") { return $true }
    if ($Text -match "(境界名称|因果|本世器物|最近记忆|轮回传承|当前世界|本世人脉|隐藏设定|轮回余烬|本命法宝器纹|器痕归灵|当前继承的传承|天机底线|近期因果摘要|下一处因果钩子|未收束线头|人情压力|NPC近况|势力压力)\s*[:：]") { return $true }
